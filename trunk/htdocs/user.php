...
$op = (isset($_GET['op']))?trim(filter_input(INPUT_GET, $_GET['op'])):((isset($_POST['op']))?trim(filter_input(INPUT_GET, $_POST['op'])):'main');
...
if($op == 'main')
{
	if(!$xoopsUser)
	{
		...
		if($isbb && (!$INFO['xbbc_reg'] or empty($INFO['xbbc_reg'])))
		{
			@header('Location: '.ICMS_URL.'/modules/ipboard/index.php?act=Reg&CODE=10');
			exit();
		}
...
if($op == 'logout')
{
	...
	setcookie('autologin_uname', '', time() - 3600, $xoops_cookie_path, '', 0, 0);
	setcookie('autologin_pass', '', time() - 3600, $xoops_cookie_path, '', 0, 0);
	...
	if(preg_match('/ipboard/', $_SERVER['HTTP_REFERER'])) {$url=$_SERVER['HTTP_REFERER'];}
	else {$url='index.php';}
	redirect_header($url, 1, $message);
	// End of Im-IPB Core edit
}

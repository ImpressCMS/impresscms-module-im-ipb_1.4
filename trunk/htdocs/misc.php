...
$action = isset(filter_input(INPUT_GET, $_GET['action'])) ? trim(filter_input(INPUT_GET, $_GET['action'])) : '';
$action = isset(filter_input(INPUT_POST, $_POST['action'])) ? trim(filter_input(INPUT_POST, $_POST['action'])) : $action;
$type = isset($_GET['type']) ? trim(filter_input(INPUT_GET, $_GET['type'])) : '';
$type = isset($_POST['type']) ? trim(filter_input(INPUT_POST, $_POST['type'])) : $type;
$op = isset($_POST['op']) ? trim(filter_input(INPUT_POST, $_POST['op'])) : $op;
...
if($action == 'showpopups')
{
	...
	switch($type)
	{
		...
		break;
		case 'friend':
			if(!$GLOBALS['xoopsSecurity']->check() || !isset($op) || $op == 'sendform') {
				if($xoopsUser)
				{
		...
		case 'online':
			...
			for($i = 0; $i < $count; $i++)
			{
				// Im-IPB Core edit - online users fix
				if($isbb)
				{
					$anons = $xoopsDB->fetchArray($xoopsDB->query("SELECT member_id as uid FROM ".$xoopsDB->prefix('session')." WHERE login_type='1' AND member_id=".$onlines[$i]['online_uid'].""));
					if($onlines[$i]['online_uid'] == 0 or !empty($anons['uid'])) {$onlineUsers[$i]['user'] = '';}
					else {$onlineUsers[$i]['user'] =& new XoopsUser($onlines[$i]['online_uid']);}
				}
				// End of Im-IPB Core edit

				elseif($onlines[$i]['online_uid'] == 0) {$onlineUsers[$i]['user'] = '';}
				else {$onlineUsers[$i]['user'] =& new XoopsUser($onlines[$i]['online_uid']);}

<?php
// $Id: modulesadmin.php 558 2006-06-20 06:35:23Z skalpa $
/*
// ------------------------------------------------------------------------
-+ Update: 12-SEP-2006
...
function xoops_module_list()
{
    ...
    foreach ( $installed_mods as $module ) {
        ...
        if ( $module->getVar('hasadmin') == 1 && $module->getVar('isactive') == 1) {
            echo '<a href="'.ICMS_URL.'/modules/'.$module->getVar('dirname').'/'.$module->getInfo('adminindex').'"><img src="'.ICMS_URL.'/modules/'.$module->getVar('dirname').'/'.$module->getInfo('image').'" alt="'.$module->getVar('name', 'E').'" border="0" /></a><br /><input type="text" name="newname['.$module->getVar('mid').']" value="'.$module->getVar('name', 'E').'" maxlength="150" size="20" />';
        } else {
            echo '<img src="'.ICMS_URL.'/modules/'.$module->getVar('dirname').'/'.$module->getInfo('image').'" alt="'.$module->getVar('name', 'E').'" border="0" /><br /><input type="text" name="newname['.$module->getVar('mid').']" value="'.$module->getVar('name', 'E').'" maxlength="150" size="20" />';
        }
        ...
        if ($module->getVar('dirname') != 'system' && $module->getVar('isactive') == 1) {
            ...
            $extra = '<a href="'.ICMS_URL.'/modules/system/admin.php?fct=modulesadmin&amp;op=update&amp;module='.$module->getVar('dirname').'"><img src="'.ICMS_URL.'/modules/system/images/update.gif" alt="'._MD_AM_UPDATE.'" /></a>';
            ...
            $extra = '<a href="'.ICMS_URL.'/modules/system/admin.php?fct=modulesadmin&amp;op=update&amp;module='.$module->getVar('dirname').'"><img src="'.ICMS_URL.'/modules/system/images/update.gif" alt="'._MD_AM_UPDATE.'" /></a>&nbsp;<a href="'.ICMS_URL.'/modules/system/admin.php?fct=modulesadmin&amp;op=uninstall&amp;module='.$module->getVar('dirname').'"><img src="'.ICMS_URL.'/modules/system/images/uninstall.gif" alt="'._MD_AM_UNINSTALL.'" /></a>';
        } else {
            echo '</td><td><input type="checkbox" name="newstatus['.$module->getVar('mid').']" value="1" checked="checked" /><input type="hidden" name="oldstatus['.$module->getVar('mid').']" value="1" />';
            $extra = '<a href="'.ICMS_URL.'/modules/system/admin.php?fct=modulesadmin&amp;op=update&amp;module='.$module->getVar('dirname').'"><img src="'.ICMS_URL.'/modules/system/images/update.gif" alt="'._MD_AM_UPDATE.'" /></a>';
        }
        ...
        <td>".$extra."&nbsp;<a href='javascript:openWithSelfMain(\"".ICMS_URL."/modules/system/admin.php?fct=version&amp;mid=".$module->getVar('mid')."\",\"Info\",300,230);'>";
        echo '<img src="'.ICMS_URL.'/modules/system/images/info.gif" alt="'._INFO.'" /></a><input type="hidden" name="module[]" value="'.$module->getVar('mid').'" /></td>
        </tr>
        ';
	...
	foreach($dirlist as $file){
        ...
        if ( !in_array($file, $listed_mods) ) {
            ...
            echo '<tr class="'.$class.'" align="center" valign="middle">
            <td align="center" valign="bottom"><img src="'.ICMS_URL.'/modules/'.$module->getInfo('dirname').'/'.$module->getInfo('image').'" alt="'.htmlspecialchars($module->getInfo('name')).'" border="0" /></td>
            <td align="center">'.round($module->getInfo('version'), 2).'</td>
            <td>
            <a href="'.ICMS_URL.'/modules/system/admin.php?fct=modulesadmin&amp;op=install&amp;module='.$module->getInfo('dirname').'"><img src="'.ICMS_URL.'/modules/system/images/install.gif" alt="'._MD_AM_INSTALL.'" /></a>';
            echo "&nbsp;<a href='javascript:openWithSelfMain(\"".ICMS_URL."/modules/system/admin.php?fct=version&amp;mid=".$module->getInfo('dirname')."\",\"Info\",300,230);'>";
            echo '<img src="'.ICMS_URL.'/modules/system/images/info.gif" alt="'._INFO.'" /></a></td></tr>
            ';
            unset($module);
            $count++;
        }
...
function xoops_module_uninstall($dirname)
{
    ...
    if ($module->getVar('dirname') == 'system') {
        ...
        if (!$module_handler->delete($module)) {
			...
			//<<<---------------------------------------------
	        //-+- Making XOOPS's module has ability to drop 	        
	        //-+- tables as we like when uninstalling module.
	        //-+- By Koudanshi
	        //<<<---------------------------------------------
			$dropsqlfile =& $module->getInfo('dropsqlfile');
			if ($dropsqlfile != false && is_array($dropsqlfile)) {
	
				$sql_file_path = ICMS_ROOT_PATH."/modules/".$dirname."/".$dropsqlfile[XOOPS_DB_TYPE];
				if (!file_exists($sql_file_path)) {
					$errs[] = "SQL file not found at <b>$sql_file_path</b>";
					$error = true;
				} else {

<?php
// $Id: main.php 653 2006-08-21 05:16:44Z skalpa $
/*
// ------------------------------------------------------------------------
-+ Update: 12-SEP-2006
...
-+ Please! don't bother XOOPS GROUP.
-+ ========================================
\\ ------------------------------------------------------------------------
*/
//  ------------------------------------------------------------------------ //
    ...
    if ($op == 'save') {
        ...
        if ($count > 0) {
            for ($i = 0; $i < $count; $i++) {
				...
				//<<<---------------------------------------------
				//-+- IPM Registration Activation type compatible 
				//-+- Koudanshi
				//<<<---------------------------------------------
				if ($isbb and $config->getVar('conf_name')=="activation_type") {
					...
					if ( ! file_exists($require) )
						die ("Cannot locate the file <font color = red>'conf_global.php'</font>. This should be uploaded into the same directory as this script!");		
					
					@chmod( $require, 0777 );
					
					if (!is_writeable($require))
						die ("Cannot write to <font color = red> 'conf_global.php'</font>. Please adjust the permissions to allow this script to write to the file. if in doubt, CHMOD via FTP to 0777");
			...
			//			print("Updated <font color = blue> 'conf_global.php' </font>sucessfull. <br> We are recommend you go AdminCp of IPBM to set config of your site in the first time.");
						fclose($fh);
					}
					else
					{
						print("Could not write to <font color = blue> 'conf_global.php'</font>");
						exit;
					}
				}
				//>>>---------------------------------------------				

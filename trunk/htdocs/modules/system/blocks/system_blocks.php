<?php
// $Id: system_blocks.php 647 2006-08-09 05:40:11Z phppp $
/*
// ------------------------------------------------------------------------
-+ Update: 12-SEP-2006
...
// Hack by Koudanshi -- Language autologin add
include_once (ICMS_ROOT_PATH."/modules/ipboard/language/".$xoopsConfig['language']."/blocks.php");
...
function b_system_online_show()
{
    ...
    if (false != $onlines) {
        ...
        for ($i = 0; $i < $total; $i++) {
			...
			//<<<--------------------------------------------
			//-+- IPBM anonymous hack by Koudanshi
			//<<<--------------------------------------------
			if ($isbb) {
				$anons = $xoopsDB->fetchArray($xoopsDB->query("SELECT member_id as uid FROM ".$xoopsDB->prefix('session')." WHERE login_type='1' AND member_id>0 AND member_id=".$onlines[$i]['online_uid'].""));
				if ($onlines[$i]['online_uid'] > 0 and $anons['uid'] > 0) {
					$anon_users++;
				} else if ($onlines[$i]['online_uid'] > 0 and !isset($anons['uid'])) {
					$members .= ' <a href="'.ICMS_URL.'/userinfo.php?uid='.$onlines[$i]['online_uid'].'">'.$onlines[$i]['online_uname'].'</a>,';
					$users++;
				} else {
					$guests++;
				}
		...
		//<<<--------------------------------------------
		//-+- IPBM anonymous display -- Koudanshi
		//<<<--------------------------------------------
			$block['lang_anons'] = _MB_IPBOARD_ANONS;
			$block['anons'] 	 = $anon_users;
		// ------------------
...
function b_system_login_show()
{
    ...
    if (!$xoopsUser) {
		...
		// Koudanshi hack Autologin Language
		$block['lang_reme']  = _MB_IPBOARD_REME;
		$block['lang_anons'] = _MB_IPBOARD_ANONS;
    // End		
...
function b_system_waiting_show()
{
    ...
    if ($module_handler->getCount(new Criteria('dirname', 'mylinks'))) {
        ...
        if ( $result ) {
            $block['modules'][1]['adminlink'] = ICMS_URL."/modules/mylinks/admin/index.php?op=listNewLinks";
        ...
        if ( $result ) {
            $block['modules'][2]['adminlink'] = ICMS_URL."/modules/mylinks/admin/index.php?op=listBrokenLinks";
        ...
        if ( $result ) {
            $block['modules'][3]['adminlink'] = ICMS_URL."/modules/mylinks/admin/index.php?op=listModReq";
    ...
    if ($module_handler->getCount(new Criteria('dirname', 'mydownloads'))) {
        ...
        if ( $result ) {
            $block['modules'][4]['adminlink'] = ICMS_URL."/modules/mydownloads/admin/index.php?op=listNewDownloads";
        ...
        if ( $result ) {
            $block['modules'][5]['adminlink'] = ICMS_URL."/modules/mydownloads/admin/index.php?op=listBrokenDownloads";
        ...
        if ( $result ) {
            $block['modules'][6]['adminlink'] = ICMS_URL."/modules/mydownloads/admin/index.php?op=listModReq";
    ...
    if (!empty($options[3])) {
        ...
        if ($xoopsDB->getRowsNum($result) > 0) {
            ...
            while  ($userinfo = $xoopsDB->fetchArray($result)) {
				...
				//<<<--------------------------------------------				
				//-+- IPBM avatar hack -- Koudanshi
				//<<<--------------------------------------------
				$avatar = XOOPS_UPLOAD_URL.'/'.$userinfo['user_avatar'];
				$avatar = str_replace ( XOOPS_UPLOAD_URL.'/http://',"http://",$avatar);
				//>>>--------------------------------------------
                if (isset($xoopsUser) && is_object($xoopsUser)) {
                    $block['groups'][$i]['users'][] = array('id' => $userinfo['uid'], 'name' => $myts->htmlspecialchars($userinfo['uname']), 'msglink' => "<a href=\"javascript:openWithSelfMain('".ICMS_URL."/pmlite.php?send2=1&amp;to_userid=".$userinfo['uid']."','pmlite',450,370);\"><img src=\"".ICMS_URL."/images/icons/pm_small.gif\" border=\"0\" width=\"27\" height=\"17\" alt=\"\" /></a>", 'avatar' => XOOPS_UPLOAD_URL.'/'.$userinfo['user_avatar']);
                } else {
                    if ($userinfo['user_viewemail']) {
                        $block['groups'][$i]['users'][] = array('id' => $userinfo['uid'], 'name' => $myts->htmlspecialchars($userinfo['uname']), 'msglink' => '<a href="mailto:'.$userinfo['email'].'"><img src="'.ICMS_URL.'/images/icons/em_small.gif" border="0" width="16" height="14" alt="" /></a>', 'avatar' => XOOPS_UPLOAD_URL.'/'.$userinfo['user_avatar']);
                    } else {
...
function b_system_newmembers_show($options)
{
    ...
    for ($i = 0; $i < $count; $i++) {
			...
			//-+- IPBM avatar hack
			//<<<--------------------------------------------
			$avatar = XOOPS_UPLOAD_URL.'/'.$newmembers[$i]->getVar('user_avatar');
			$avatar = str_replace ( XOOPS_UPLOAD_URL.'/http://','http://',$avatar);
			//<<<--------------------------------------------
...
function b_system_topposters_show($options)
{
    ...
    for ($i = 0; $i < $count; $i++) {
        $block['users'][$i]['rank'] = $i+1;
			...
			//-+- IPBM avatar hack
			//<<<--------------------------------------------
			$avatar = XOOPS_UPLOAD_URL.'/'.$topposters[$i]->getVar('user_avatar');
			$avatar = str_replace ( XOOPS_UPLOAD_URL.'/http://','http://',$avatar);
			//>>>--------------------------------------------

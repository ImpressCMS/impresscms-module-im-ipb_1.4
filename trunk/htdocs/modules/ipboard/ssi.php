...
function do_news()
{
	...
	$DB->query("SELECT m.uname as member_name, m.uid as member_id,m.title as member_title, m.user_avatar, m.avatar_size, m.posts, t.*, p.*, g.g_dohtml, f.use_html
	            FROM ibf_topics t
	            	LEFT JOIN ibf_posts p ON (p.new_topic=1 AND p.topic_id=t.tid)
	            	LEFT JOIN ibf_members m ON (m.uid=t.starter_id)

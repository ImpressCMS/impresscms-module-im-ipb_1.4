...
//+-- XOOPS Configure -- Kou added --/=
include ("./../../mainfile.php");  
//+-- Module initial
$mod_handler = & xoops_gethandler('module'); 
//--|-- NEWS
if (is_object($news_mod = & $mod_handler->getByDirName('news'))){
  $news_isactive = $news_mod->getVar('isactive');
} else {
  $news_isactive = 0;
}
//--|-- IPBOARD
$ipb_mod = & $mod_handler->getByDirName('ipboard');
$ipb_mid = $ipb_mod->getVar('mid');
$ipbm_q  = $xoopsDB->fetcharray($xoopsDB->query("SELECT gperm_id FROM ".$xoopsDB->prefix("group_permission")." WHERE gperm_name='module_read' AND gperm_itemid='".$ipb_mid."' AND gperm_groupid=3 LIMIT 1"));  
$ipb_isactive = intval($ipbm_q['gperm_id']);
//+-- XOOPS Configure -- Kou added --/=
...
if ($IN['login'] != 'yes') {
	...
	else
	{
		...
		else
		{
			...
			$DB->query("SELECT * FROM ibf_members WHERE uid='".$row['MEMBER_ID']."'");
			$MEMBER = $DB->fetch_row();
...
else
{
	...
	$DB->query("SELECT uname, pass, uid, mgroup FROM ibf_members WHERE LOWER(uname)='".$username."'");
	$mem = $DB->fetch_row();
	...
	else
	{
		...
		else
		{
			...
			$ADMIN->html .= "<tr><td id='tdrow1'><meta HTTP-EQUIV=\"Content-Type\" content=\"text/html; charset=".$INFO['xbbc_charset']."\"><meta http-equiv='refresh' content='1; url=".$INFO['board_url']."/admin.".$INFO['php_ext']."?adsess=".$IN['AD_SESS']."'><a href='".$INFO['board_url']."/admin.".$INFO['php_ext']."?adsess=".$IN['AD_SESS']."'>( Click here if you do not wish to wait )</a></td></tr>";
...
function do_login($message="") {
	...
	if ( $mid > 0 )
	{
		$DB->query("SELECT m.uid, m.uname, m.mgroup, g.g_access_cp FROM ibf_members m, ibf_groups g WHERE m.uid=$mid AND g.g_id=m.mgroup AND g.g_access_cp=1");

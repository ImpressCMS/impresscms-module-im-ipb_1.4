...
//<<<--------------------------------------------
//-+- Cheking the first install -- Koudanshi
//<<<--------------------------------------------
$require = ROOT_PATH."conf_global.php";

if ( !file_exists($require) )
	die ("Cannot locate file <font color = red>'conf_global.php'</font>. This should be uploaded into the same directory as this script!");

include ($require);

if ( count($INFO) < 100 ) {
	if (!include (ROOT_PATH."sm_install.php"))
		die ("Cannot locate file <font color = red>'sm_install.php'</font>. This should be uploaded into the same directory as this script!");
...
class info {
	...
	var $lang       = "";
	var $server_load= 0;
	var $version    = "v1.31 Final";
	var $version_bb = "v1.4J";
	var $lastclick  = "";
	var $location   = "";
	var $debug_html = "";
	var $perm_id    = "";
	...
	function info() {
		...
		$this->vars['AVATARS_URL']     = ICMS_URL.'/uploads';
		$this->vars['EMOTICONS_URL']   = ICMS_URL.'/uploads';
...
if ($ibforums->input['act'] != 'Login' and $ibforums->input['act'] != 'Reg' and $ibforums->input['act'] != 'Attach' and $ibforums->input['module'] != 'subscription')
{
	...
	if ( (! $ibforums->member['uid']) and ($ibforums->vars['force_login'] == 1) )
	{

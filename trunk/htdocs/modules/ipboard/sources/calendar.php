...
class calendar {
    ...
    function make_event_html($event)
    {
        ...
        else if ($ibforums->member['uid'] == $event['userid'])
        {
        	$edit_button = $this->html->cal_edit_del_button($event['eventid']);
        }
        ...
        $DB->query("SELECT m.uid, m.uname, m.mgroup, m.posts, m.joined, m.user_avatar, m.avatar_size, ".
                   "g.g_id, g.g_title FROM ibf_members m, ibf_groups g WHERE m.uid='".$event['userid']."' AND g.g_id=m.mgroup");
    ...
    function make_birthday_html($month, $day, $switch=0) {
        ...
        $DB->query("SELECT uid, uname, bday_year from ibf_members WHERE bday_month='".$month."' and bday_day='$day'");

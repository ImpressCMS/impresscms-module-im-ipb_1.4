...
class UserCP {
    ...
    // Get the sync module
  	...
  	// Check viewing permissions, etc
		...
		if ( empty($this->member['uid']) or $this->member['uid'] == "" or $this->member['uid'] == 0 )
		{
    	...
    	$DB->query("SELECT m.*, m.user_sig as signature, me.notes,me.ta_size,me.photo_type,me.photo_location,me.photo_dimensions
    			   FROM ibf_members m
    ...
    $print->do_output( array( 'TITLE' => $this->page_title, 'JS' => 1, NAV => $this->nav ) );
 	...
 	function do_pass_change()
 	{
 		...
 		$DB->query("UPDATE ibf_members SET pass='$md5_pass' WHERE uid='".$this->member['uid']."'");
 		...
 		if ( USE_MODULES == 1 )
		{
    		...
    		$this->modules->on_pass_change($ibforums->member['uid'], $new_pass);
   		}
 	...
 	function do_email_change()
 	{
 		...
 		if ( $ibforums->member['pass'] !=  md5($ibforums->input['password']) )
 		{
 			$std->Error( array( 'LEVEL' => 1, 'MSG' => 'wrong_pass' ) );
 		}
		...
		{
			$DB->query("SELECT uid FROM ibf_members WHERE email='".$email_one."'");
		...
		{
			$this->modules->register_class(&$this);
    		$this->modules->on_email_change($ibforums->member['uid'], $email_one);
   		}
		...
		if ($ibforums->vars['reg_auth_type'])
		{
			...
			$DB->query("UPDATE ibf_members SET mgroup=".$ibforums->vars['auth_group'].", email='$email_one' WHERE uid=".$this->member['uid']);

			// Update their session with the new member group

			if ($ibforums->session_id)
			{
				$DB->query("UPDATE ibf_sessions SET member_name='', member_id=0, member_group=".$ibforums->vars['guest_group']." WHERE member_id=".$this->member['uid']." and sess_id='".$ibforums->session_id."'");
 			}
			...
			$this->email->build_message( array(
												...
												'THE_LINK'     => $this->base_url_nosess."?act=Reg&CODE=03&type=newemail&uid=".$this->member['uid']."&aid=".$validate_key,
												'ID'           => $this->member['uid'],
		...
		else
		{
			...
			$DB->query("UPDATE ibf_members SET email='$email_one' WHERE uid='".$this->member['uid']."'");
 	...
 	function tracker() {
 		...
 		$DB->query("SELECT s.trid, s.member_id, s.topic_id, s.last_sent, s.start_date as track_started, t.*, f.id as forum_id, f.name as forum_name, f.read_perms "
 		          ...
 		          ."WHERE s.member_id='".$this->member['uid']."' AND t.tid=s.topic_id AND f.id=t.forum_id $date_query"
 	...
 	function board_prefs() {
		...
		if ($ibforums->vars['topicpage_contents'] == "")
		{
 		...
 		$view_sig .= $this->member['attachsig'] ? "<option value='1' selected='selected'>".$ibforums->lang['yes']."</option>\n<option value='0'>".$ibforums->lang['no']."</option>"
 											   : "<option value='1'>".$ibforums->lang['yes']."</option>\n<option value='0' selected='selected'>".$ibforums->lang['no']."</option>";
    ...
    //+----------------------------------------------
    //| XOOPS's user setting add-on
    //| By Koudanshi
    //+----------------------------------------------
    
    // Set up XOOPS's setting lang
  	if ( file_exists(ICMS_ROOT_PATH."/language/".$xoopsConfig['language']."/user.php") ) {
  		include_once ICMS_ROOT_PATH."/language/".$xoopsConfig['language']."/user.php";
  	} else {
  		include ICMS_ROOT_PATH."/language/english/user.php";
  	}
  	if ( file_exists(ICMS_ROOT_PATH."/language/".$xoopsConfig['language']."/notification.php") ) {
  		...
  		include ICMS_ROOT_PATH."/language/english/notification.php";
  	}
  	include_once ICMS_ROOT_PATH . '/include/notification_constants.php';
  	
    $ibforums->lang['com_disp']       = "<b>Portal - </b> "._US_CDISPLAYMODE;
    $ibforums->lang['com_order']      = "<b>Portal - </b> "._US_CSORTORDER;
    $ibforums->lang['notify_method']  = "<b>Portal - </b> "._NOT_NOTIFYMETHOD;
    $ibforums->lang['notify_mode']    = "<b>Portal - </b> "._NOT_NOTIFYMODE;
    //+----------------------------------------------
    
    $com_disp      = "<select name='COM_DISP' class='forminput'>";
    $com_order     = "<select name='COM_ORDER' class='forminput'>";
    $notify_method = "<select name='NOTIFY_METHOD' class='forminput'>";
    $notify_mode   = "<select name='NOTIFY_MODE' class='forminput'>";

    // Comment display setting    
    $com_disp_used1 = $com_disp_used2 = $com_disp_used3 = "";
    if ($this->member['umode'] == "nest") {
      $com_disp_used1 .= "selected";
    } elseif ($this->member['umode'] == "flat") {
        $com_disp_used2 .= "selected";
      } elseif ($this->member['umode'] == "thread") {
          $com_disp_used3 .= "selected";
        }
	  $com_disp .= "<option value='nest'   $com_disp_used1>"._NESTED."   </option>\n";
	  $com_disp .= "<option value='flat'   $com_disp_used2>"._FLAT."     </option>\n";
	  $com_disp .= "<option value='thread' $com_disp_used3>"._THREADED." </option>\n";
    
    // Comment Order 
 		$com_order .= $this->member['uorder'] == 0 ? "<option value='0' selected='selected'>"._OLDESTFIRST."</option>\n<option value='1'>"._NEWESTFIRST."</option>"
									   : "<option value='0'>"._OLDESTFIRST."</option>\n<option value='1' selected='selected'>"._NEWESTFIRST."</option>";

    // Notify setting
    $notify_method_used1 = $notify_method_used2 = $notify_method_used3 = "";
    if ($this->member['notify_method'] == XOOPS_NOTIFICATION_METHOD_DISABLE) {
      $notify_method_used1 .= "selected";
    } elseif ($this->member['notify_method'] == XOOPS_NOTIFICATION_METHOD_PM) {
        $notify_method_used2 .= "selected";
      } elseif ($this->member['notify_method'] == XOOPS_NOTIFICATION_METHOD_EMAIL) {
          $notify_method_used3 .= "selected";
        }
	  $notify_method .= "<option value='".XOOPS_NOTIFICATION_METHOD_DISABLE."'  $notify_method_used1>"._NOT_METHOD_DISABLE."</option>\n";
	  $notify_method .= "<option value='".XOOPS_NOTIFICATION_METHOD_PM."'       $notify_method_used2>"._NOT_METHOD_PM."</option>\n";
	  $notify_method .= "<option value='".XOOPS_NOTIFICATION_METHOD_EMAIL."'    $notify_method_used3>"._NOT_METHOD_EMAIL."</option>\n";
    
    // Notify mode
    $notify_mode_used1 = $notify_mode_used2 = $notify_mode_used3 = "";
    if ($this->member['notify_mode'] == XOOPS_NOTIFICATION_MODE_SENDALWAYS) {
      $notify_mode_used1 .= "selected";
    } elseif ($this->member['notify_mode'] == XOOPS_NOTIFICATION_MODE_SENDONCETHENDELETE) {
        $notify_mode_used2 .= "selected";
      } elseif ($this->member['notify_mode'] == XOOPS_NOTIFICATION_METHOD_EMAIL) {
          $notify_mode_used3 .= "selected";
        }
	  $notify_mode .= "<option value='".XOOPS_NOTIFICATION_MODE_SENDALWAYS."'         $notify_mode_used1>"._NOT_MODE_SENDALWAYS."       </option>\n";
	  $notify_mode .= "<option value='".XOOPS_NOTIFICATION_MODE_SENDONCETHENDELETE."' $notify_mode_used2>"._NOT_MODE_SENDONCE."         </option>\n";
	  $notify_mode .= "<option value='".XOOPS_NOTIFICATION_MODE_SENDONCETHENWAIT."'   $notify_mode_used3>"._NOT_MODE_SENDONCEPERLOGIN." </option>\n";
    
    //+----------------------------------------------

 		$this->output .= $this->html->settings_end( array (
    //+----------------------------------------------
 		// Koudanshi -- XOOPS's user setting
    //+---------------------------------------------- 		
 		                          'COM_DISP'      => $com_disp."</select>",
 		                          'COM_ORDER'     => $com_order."</select>",
 		                          'NOTIFY_METHOD' => $notify_method."</select>",
 		                          'NOTIFY_MODE'   => $notify_mode."</select>",
    //+----------------------------------------------
 		                          'IMG'  => $view_img."</select>",
 															'SIG'  => $view_sig."</select>",
 															'AVA'  => $view_ava."</select>",
 	...
 	function email_settings() {
 		...
 		foreach ( array(user_mailok, email_full, email_pm, auto_track) as $k )
 		{
        ...
        // --------------------------------------------
        // By Koudanshi
        // Convert between XOOPS and IPB email options
        // --------------------------------------------
 	...
 	function avatar_gallery()
 	{
		...
		$up_dir = ICMS_ROOT_PATH.'/uploads/';
 		...
 		$dh = opendir( $up_dir."avatars/" );

 		while ( $file = readdir( $dh ) )
 		{
			if ( is_dir( $up_dir."avatars/".$file ) )
			{
				if ( $file != "." && $file != ".." )
				{
		...
		$dh = opendir( $up_dir."avatars".$av_cat_real);

 		while ( $file = readdir( $dh ) )
 		{
 			if ( ! preg_match( "/^..?$|^index|^\.ds_store|^\.htaccess/i", $file ) )
 			{
 				if ( is_file( $up_dir."avatars".$av_cat_real."/".$file) )
 				{
 					if ( preg_match( "/\.(gif|jpg|jpeg|png|swf)$/", $file ) )
 					{
 						$av_gall_images[] = $file;
 					}
 				}
 	...
 	function avatar()
 	{
		...
		$up_dir = ICMS_ROOT_PATH.'/uploads/';
 		...
 		$my_avatar = $my_avatar ? $my_avatar : 'blank.gif';
 		...
 		if ( ($this->member['user_avatar'] != "") and ($this->member['user_avatar'] != "blank.gif") )
 		{
 			if ( preg_match( "/^cavt/", $this->member['user_avatar'] ) )
 			{
 				$avatar_type = "upload";
 			}
			else if ( !preg_match( "/^http/i", $this->member['user_avatar'] ) )
			{
				$avatar_type = "gallery";
			}
			else
		...
		//+----------------------------------------
 		...
 		else
 		{
 			$DB->query("INSERT INTO ibf_member_extra (id, notes, ta_size) ".
 			           " VALUES ('".$this->member['uid']."', '".$ibforums->input['notes']."', '".$ibforums->input['ta_size']."')");
 		}

...
class ad_settings {
    ...
    $this->mysql_version = (int)sprintf('%d%02d%02d', $one, $two, intval($three));
		...
		switch($IN['code'])
		{
			...
			case 'url':
				...
				break;
			case 'dourl':
				$this->save_config( array ( 'number_format', 'html_dir','safe_mode_skins', 'board_name','board_url','home_name','home_url',
	...
	function edit_emoticons()
	{
		...
		$dh = opendir( $INFO['html_dir'].'/../../../uploads' ) or die("Could not open the emoticons directory for reading, check paths and permissions");
	...
	function emoticons()
	{
		...
		$ADMIN->page_detail = "You may add/edit or remove emoticons in this section.<br>You can only choose emoticons that have been uploaded into the ,<font color=blue> 'yoursite.com/xoops/html/uploads' </font>directory.<br><br>Clickable refers to emoticons that are in the posting screens 'Clickable Emoticons' table.";
...
/*
		$ADMIN->html .= $SKIN->add_td_row( array( "<b><i>DISABLE</I> GZIP encoding?</b><br>(GZIP enables faster page transfer and lower bandwidth use)" ,
										  $SKIN->form_yes_no( "disable_gzip", 1 )
								 )      );
*/

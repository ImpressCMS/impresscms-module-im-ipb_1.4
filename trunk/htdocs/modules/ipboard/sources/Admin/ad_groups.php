	...
	function remove_mask()
	{
		...
		else
		{
			...
			else
			{
				...
				$DB->query("UPDATE ibf_members SET org_perm_id='".implode(",",$new_arr)."' WHERE uid=".intval($IN['id']));
			}
		...
		$ADMIN->html .= $SKIN->add_td_row( array( "Removed the custom permission mask from <b>{$mem['uname']}</b>.",
										 )      );
			...
			foreach( explode( ",", $r['org_perm_id'] ) as $pid )
			{
				if ( $pid == $IN['id'] )
				{
					...
					$ADMIN->html .= $SKIN->add_td_row( array( "<div style='font-weight:bold;font-size:11px;padding-bottom:6px;margin-bottom:3px;border-bottom:1px solid #000'>{$r['uname']}</div>
															   ...
															   <li>Email: {$r['email']}
															   $extra" ,
															  "&#149;&nbsp;<a href='{$SKIN->base_url}&amp;act=group&amp;code=remove_mask&amp;id={$r['uid']}&amp;pid=$pid' title='Remove this mask from the user (will not remove all if they have multimasks'>Remove This Mask</a>
															   <br />&#149;&nbsp;<a href='{$SKIN->base_url}&amp;act=group&amp;code=remove_mask&amp;id={$r['uid']}&amp;pid=all' title='Remove all user masks'>Remove All Masks</a>
															   <br /><br />&#149;&nbsp;<a href='javascript:pop_close_and_stop(\"{$r['uid']}\");'>Edit Member</a>",
													 )      );
				}
			}
		}

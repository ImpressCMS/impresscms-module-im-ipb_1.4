...
class ad_adlogs {
	...
	function view()
	{
		...
		if ($IN['search_string'] == "")
		{
			...
			$DB->query("SELECT m.*, mem.uid, mem.uname FROM ibf_admin_logs m, ibf_members mem
		...
		else
		{
			...
			$DB->query("SELECT m.*, mem.uid, mem.uname FROM ibf_admin_logs m, ibf_members mem
					    WHERE m.member_id=mem.uid AND $dbq ORDER BY m.ctime DESC LIMIT $start, 20");
		...
		if ( $DB->get_num_rows() )
		{
			while ( $row = $DB->fetch_row() )
			{
				...
				$ADMIN->html .= $SKIN->add_td_row( array( "<b>{$row['uname']}</b>",
	...
	function list_current()
	{
		...
		$DB->query("SELECT m.*, mem.uname, count(m.id) as act_count FROM ibf_admin_logs m, ibf_members mem WHERE m.member_id=mem.uid GROUP BY m.member_id ORDER BY act_count DESC");

		while ( $r = $DB->fetch_row() )
		{

			$ADMIN->html .= $SKIN->add_td_row( array( "<b>{$r['uname']}</b>",

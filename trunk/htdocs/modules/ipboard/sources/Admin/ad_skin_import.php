...
class ad_langs {
    ...
    //-- mod_safe_mode begin -- by Peter

    if (SAFE_MODE_ON == 1 and $INFO['safe_mode_skins'] == 1) {
         switch($IN['code']) {
             case 'doimport':
                 $this->doimport();
                 break;
    ...
    //+-------------------------------
    //| Koudanshi hack skins Stuff
    //+-------------------------------
    $charset="charset=".$INFO['xbbc_charset']."\"";
		$user_stuff_new   = "   <!--{USER_STUFF}-->\n</table>\n<div class='tableborder'>\n";
		$user_stuff_old   = "</table>\n<div class='tableborder'>\n";
		$xsetting_old = "<table width=\"100%\" cellpadding=\"4\">\n";
		$xsetting_new = "<table width=\"100%\" cellpadding=\"4\">\n".
                  "<tr>\n".
                  "  <td width='70%'>{\$ibforums->lang['com_disp']}</td>\n".
                  "  <td align='left'>{\$data[COM_DISP]}</td>\n".
                  "</tr>\n".
                  "<tr>\n".
                  "  <td width='70%'>{\$ibforums->lang['com_order']}</td>\n".
                  "  <td align='left'>{\$data[COM_ORDER]}</td>\n".
                  "</tr>\n".
                  "<tr>\n".
                  "  <td width='70%'>{\$ibforums->lang['notify_method']}</td>\n".
                  "  <td align='left'>{\$data[NOTIFY_METHOD]}</td>\n".
                  "</tr>\n".
                  "<tr>\n".
                  "  <td width='70%'>{\$ibforums->lang['notify_mode']}</td>\n".
                  "  <td align='left'>{\$data[NOTIFY_MODE]}</td>\n".
                  "</tr>\n";
    // -------------------------------
        ...
        //| Koudanshi hack skins Stuff
        //+-------------------------------
        if ($data['func_name'] == 'show_profile') {
          // Remove the old one
          $data['content'] = str_replace("<!--{USER_STUFF}-->\n", "\n", $data['content']);         
          // UserProfile skin
  		    $data['content'] = str_replace($user_stuff_old, $user_stuff_new, $data['content']) ;
        }
        if ($data['func_name'] == 'avatar_gallery_cell_row') {
          // Usercp skin
          $data['content'] = str_replace( "html/avatars", "../../uploads/avatars", $data['content']) ;                    
        }
        if ($data['func_name'] == 'settings_end') {
          // Usercp skin
          // Remove old one
          $data['content'] = str_replace( $xsetting_new, $xsetting_old, $data['content']) ;                    
          // Add new one
   		    $data['content'] = str_replace( "<table width=\"100%\" cellpadding=\"4\">\n", $xsetting_new, $data['content']) ;          
        }
        // Charset convert
        $data['content'] = preg_replace( "#charset=(.+?)\"#is", $charset, $data['content']) ;                    
        // -------------------------------
      ...
      //| Koudanshi hack skins Stuff
      //+-------------------------------
			if ($group == 'skin_profile'){
				$final .= "
				\n\nfunction ".trim(user_stuff)." (\$show) {\n".
				"global \$ibforums;\n".
				"return <<<EOF\n".
				"\$show\n".
				"\nEOF;\n}\n".
      ...
      // Charset convert
       ...
       //+-------------------------------
			...
			else
			{
				...
				//+-------------------------------
				//| Koudanshi User Stuff
				//+-------------------------------
				if ($group == 'skin_profile' and $data['func_name'] == 'custom_field')
				{
					$DB->query("INSERT INTO ibf_skin_templates (section_content, func_data, set_id, group_name, func_name, can_remove) VALUES ('\$show', '\$show', $setid, 'skin_profile', 'user_stuff', 0)");
				}
				//+-------------------------------
			}
		}
  ...
  //-- mod_safe_mode begin
  function safe_list_current() {
      global $IN, $INFO, $DB, $SKIN, $ADMIN, $std, $MEMBER, $GROUP;
  
      $lang = $std->load_words($lang, 'mod_safe_mode_lang', $INFO['default_language']==""?'en':$INFO['default_language']);
  
      $form_array = array();
      $ADMIN->page_title = $lang['import_title'];
      $ADMIN->page_detail = $lang['import_detail'];
      $files = array();
      $dir = $INFO['base_dir']."/archive_in";
      if ( is_dir($dir) ) {
           $handle = opendir($dir);
      ...
      $SKIN->td_header[] = array( $lang['name'] , "30%" );
      $SKIN->td_header[] = array( $lang['type'] , "20%" );
      $SKIN->td_header[] = array( $lang['dir_name'] , "30%" );
      $SKIN->td_header[] = array( $lang['import'] , "10%" );
      $ADMIN->html .= $SKIN->start_table( $lang['list_uploaded'] );
      if ( count($files) > 0 ) {
           foreach($files as $file) {
                $type = array(   'css' => 'Style Sheet',
                                              'image' => 'Image & Macro set',
                                              'set' => 'Skin Set Collection Directory',
                                              'wrap' => 'Board Wrapper',
                                              'tmpl' => 'Template set'
                                          );
                $rtype = preg_replace( "/^set.+?$/" , "set", $file );
                $rname = preg_replace( "/^(css|image|set|wrap|tmpl)-(.+?)\.(\S+)$/", "\\2", $file );
                $rname = preg_replace( "/_/", " ", $rname );
                $ADMIN->html .= $SKIN->add_td_row( array( "<b>$rname</b>",
                                                                            "<center>{$type[$rtype]}</center>",
  ...
  function safe_import() {
      ...
      $this->tar_file = $IN['id'];
      $this->name_translated = preg_replace( "/^(css|image|set|wrap|tmpl)-(.+?)$/", "\\2", $this->tar_file );
  ...
  function safe_set_import() {
      ...
      $this->images_dir = $images_dir."/".$next_id['images'];
      $this->skin_dir = $skins_dir."/s".$next_id['templates'];
      $set_dir = $this->work_path."/".$IN['id']."/";
  
      //------------------------------------------------------
  
      if ( file_exists($set_dir."templates_conf.inc") ) {
          require $set_dir."templates_conf.inc";
      ...
      if ($FH = fopen( $set_dir."stylesheet.css", 'r' ) ) {
          $css = fread($FH, filesize($set_dir."stylesheet.css") );
          fclose($FH);
      ...
      if ($FH = fopen( $set_dir."wrapper.html", 'r' ) ) {
          $text = fread($FH, filesize($set_dir."wrapper.html") );
          fclose($FH);
      ...
      if ($FH = fopen( $set_dir."macro.txt", 'r' ) ) {
          $data = fread($FH, filesize($set_dir."macro.txt") );
          fclose($FH);
      ...
      if ($FH = fopen( $set_dir."templates.html", 'r' ) ) {
          $data = fread($FH, filesize($set_dir."templates.html") );
          fclose($FH);
      ...
      $info = $lang['set_import_info'];
      $info = str_replace("%set_dir%", $set_dir, $info);
      $info = str_replace("%images_dir%",$this->images_dir, $info);
      $info = str_replace("%skin_dir%", $this->skin_dir, $info);
  
      $ADMIN->page_title = $lang['set_import_title'];
      $ADMIN->page_detail = $lang['set_import_detail'];
  
      $SKIN->td_header[] = array( "&nbsp;" , "100%" );
  
      $ADMIN->html .= $SKIN->start_table($lang['finalize_info']);
  
      $ADMIN->html .= $SKIN->add_td_basic( $info );
  
      $ADMIN->html .= $SKIN->add_td_spacer();
  
      $ADMIN->html .= $SKIN->add_td_basic( "<a href='{$ADMIN->base_url}&act=templ' target='body'>".$lang['link_rebuild']."</a>", "center" );
      $ADMIN->html .= $SKIN->add_td_basic( "<a href='{$ADMIN->base_url}&act=sets' target='body'>".$lang['link_skin_sets']."</a>", "center" );

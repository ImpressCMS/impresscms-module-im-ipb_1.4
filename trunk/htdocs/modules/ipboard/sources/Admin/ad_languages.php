...
-- mod_safe_mode exclude end */                
	...
	function list_current()
	{
		...
		$DB->query("select ibf_languages.*, count(ibf_members.uid) as mcount from ibf_languages left join ibf_members on(ibf_members.language=ibf_languages.ldir) where (ibf_members.language is not null or ibf_members.language = 'en') group by ibf_languages.ldir order by ibf_languages.lname");
  ...
  function safe_doimport() {
      ...
      $lang_dest_dir = $INFO['base_dir']."lang";
  
      $lang_source_dir = $INFO['base_dir']."archive_in/".$IN['lang_dir'];
  
      //------------------------------
  
      $real_name = preg_replace( "/^lang-(.+?)$/", "\\1", $IN['lang_dir'] );
      ...
      $lang_dest_dir = $lang_dest_dir."/".$new_id;
  
      $extra = array( 'lauthor' => "", 'lemail' => "" );
  
      if (file_exists($lang_source_dir."/conf.inc")) {
          require $lang_source_dir."/conf.inc";
          $extra['lauthor'] = stripslashes($config['lauthor']);
          $extra['lemail'] = stripslashes($config['lemail']);
      }
      ...
      $info = $lang['lang_imported_info'];
      $info = str_replace("%lang_source_dir%", $lang_source_dir, $info);
      $info = str_replace("%lang_dest_dir%", $lang_dest_dir, $info);
  
      $ADMIN->page_title = $lang['lang_imported_title'];
      $ADMIN->page_detail = $lang['lang_imported_detail'];
  
      $SKIN->td_header[] = array( "&nbsp;" , "100%" );
  
      $ADMIN->html .= $SKIN->start_table($lang['lang_finalize_info']);
  
      $ADMIN->html .= $SKIN->add_td_basic( $info );
  
      $ADMIN->html .= $SKIN->add_td_spacer();
  
      $ADMIN->html .= $SKIN->add_td_basic( "<a href='{$ADMIN->base_url}&act=lang' target='body'>".$lang['link_lang_sets']."</a>", "center" );
  ...
  function safe_import() {
      ...
      $ADMIN->page_detail = $lang['lang_import_detail'];
      $ADMIN->page_title = $lang['lang_import_title'];
      ...
      $ADMIN->html .= $SKIN->start_table( $lang['lang_list_info']);
      ...
      $ADMIN->html .= $SKIN->add_td_row( array( $lang['lang_list'] ,
                                                                          $SKIN->form_dropdown( "lang_dir", $form_array )
                                                                        ) );

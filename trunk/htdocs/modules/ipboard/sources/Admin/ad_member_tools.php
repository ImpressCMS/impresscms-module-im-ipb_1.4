...
class ad_member_tools {
	...
	function learn_ip()
	{
		...
		$DB->query("SELECT uid, uname, email, posts, ip_address, user_regdate FROM ibf_members WHERE ip_address{$query} ORDER BY user_regdate DESC LIMIT 250");
		...
		else
		{
			while ( $m = $DB->fetch_row() )
			{
				...
				$ADMIN->html .= $SKIN->add_td_row( array( $m['uname'] ,
														  $m['email'],
														  $m['posts'],
														  $m['ip_address'],
														  $std->get_date( $m['user_regdate'], 'SHORT' )
												 )      );
			}
		}
		...
		$DB->query("SELECT m.uid, m.uname, m.email, m.posts, m.user_regdate, p.pid, p.author_id, p.post_date, p.ip_address, p.topic_id
					FROM ibf_posts p
		...
		$DB->query("SELECT m.uid, m.uname, m.email, m.posts, m.user_regdate, p.vote_date, p.ip_address, p.tid
					FROM ibf_voters p
					 LEFT JOIN ibf_members m ON ( p.member_id=m.uid)
		...
		$DB->query("SELECT m.uid, m.uname, m.email, m.posts, m.user_regdate, p.email_date, p.from_ip_address
					FROM ibf_email_logs p
					 LEFT JOIN ibf_members m ON ( p.from_member_id=m.uid)
		...
		$DB->query("SELECT m.uid, m.uname, m.email, m.posts, m.user_regdate, p.entry_date, p.ip_address
	...
	function show_ips()
	{
		...
		if ( $IN['member_id'] )
		{
			...
			$DB->query("SELECT uid, uname, email, ip_address FROM ibf_members WHERE uid=$id");
		...
		else
		{
			...
			$DB->query("SELECT uid, uname, email, ip_address FROM ibf_members WHERE uname='$name'");
		...
		$DB->query("SELECT count(distinct(ip_address)) as cnt FROM ibf_posts WHERE author_id={$member['uid']}");
		...
		$links = $std->build_pagelinks( array( 'TOTAL_POSS'  => $count['cnt'],
											   ...
											   'BASE_URL'    => $SKIN->base_url."&act=mtools&code=showallips&member_id={$member['uid']}",
									  )      );
		
		$master = array();
		$ips    = array();
		
		$DB->query("SELECT count(ip_address) as ip, ip_address, pid, topic_id, post_date FROM ibf_posts WHERE author_id={$member['uid']} GROUP BY ip_address ORDER BY ip DESC LIMIT $st, $end");
		...
		if ( count($ips) > 0 )
		{
			$DB->query("SELECT uid, uname, ip_address FROM ibf_members WHERE ip_address IN (".implode(",",$ips).") AND uid != {$member['uid']}");
		...
		$ADMIN->html .= $SKIN->start_table( "{$member['uname']}'s IP addresses ({$count['cnt']}) matches" );
	...
	function show_index($msg="", $membername="")
	{
		...
		{
			$DB->query("SELECT uid, uname FROM ibf_members WHERE lower(uname) LIKE '{$membername}%'");

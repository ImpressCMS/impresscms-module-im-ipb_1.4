...
class ad_emaillogs
{
	...
	function view_email()
	{
		...
		$DB->query("SELECT email.*, m.uid, m.uname, mem.uid as to_id, mem.uname as to_name FROM ibf_email_logs email
					 LEFT JOIN ibf_members m ON (m.uid=email.from_member_id)
					 LEFT JOIN ibf_members mem ON (mem.uid=email.to_member_id)
		...
		$ADMIN->html .= $SKIN->add_td_row( array(
													"<strong>From:</strong> {$row['uname']} &lt;{$row['from_email_address']}&gt;
	...
	function list_current()
	{
		...
		if ( $IN['type'] != "" )
		{
			...
			switch( $IN['type'] )
			{
				...
				case 'name_from':
					...
					if ( $IN['match'] == 'loose' )
					{
						$DB->query("SELECT uid as id,uname as name FROM ibf_members WHERE uname LIKE '%{$string}%'");
					...
					{
						$DB->query("SELECT uid as id,uname as name FROM ibf_members WHERE uname='{$string}'");
				...
				case 'name_to':
					...
					else
					{
						...
						$db_query[] = 'email.to_member_id IN('.$r['uid'].')';
					}
		...
		$DB->query("SELECT email.*, m.uid, m.uname, mem.uid as to_id, mem.uname as to_name FROM ibf_email_logs email
					 LEFT JOIN ibf_members m ON (m.uid=email.from_member_id)
					 LEFT JOIN ibf_members mem ON (mem.uid=email.to_member_id) $dbe
		...
		if ( $DB->get_num_rows() )
		{
			while ( $row = $DB->fetch_row() )
			{
				...
				$ADMIN->html .= $SKIN->add_td_row( array(
														  ...
														  "<a href='{$ADMIN->base_url}&act=emaillog&code=list&type=fromid&uid={$row['uid']}' title='Show all from this member'><img src='{$SKIN->img_url}/acp_search.gif' border='0' alt='..by id'></a>&nbsp;<b><a href='{$INFO['board_url']}/index.{$INFO['php_ext']}?act=Profile&MID={$row['uid']}' title='Members profile (new window)' target='blank'>{$row['uname']}</a></b>",
														  "<a href='javascript:pop_win(\"&act=emaillog&code=viewemail&id={$row['email_id']}\",400,400)' title='Read email'>{$row['email_subject']}</a>",
														  "<a href='{$ADMIN->base_url}&act=emaillog&code=list&type=toid&uid={$row['to_id']}' title='Show all sent to this member'><img src='{$SKIN->img_url}/acp_search.gif' border='0' alt='..by id'></a>&nbsp;<a href='{$INFO['board_url']}/index.{$INFO['php_ext']}?act=Profile&MID={$row['to_id']}'  title='Members profile (new window)' target='blank'>{$row['to_name']}</a>",
														  "{$row['email_date']}",
												 )      );

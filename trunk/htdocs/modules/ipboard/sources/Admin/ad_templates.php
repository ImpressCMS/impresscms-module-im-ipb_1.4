...
class ad_settings {
	...
	function tools()
	{
		...
		{
      //-- mod_safe_mode begin
      if ($IN['resync_skins'] == '1')
          $this->tools_build_tmpl_all();
      else
      //-- mod_safe_mode end		  
	...
	function tools_build_tmpl()
	{
		...
		if ( is_dir($skin_dir) )
		{
			if ( $handle = opendir($skin_dir) )
			{
              ...
              //-- mod_safe_mode begin
              if (SAFE_MODE_ON != 1 or $INFO['safe_mode_skins'] != 1) {
              //-- mod_safe_mode end
              ...
              //-- mod_safe_mode end
                ...
                //-- mod_safe_mode begin
                if (SAFE_MODE_ON == 1 or $INFO['safe_mode_skins'] == 1) {
                    $DB->query("DELETE FROM ibf_skin_templates WHERE set_id='".$IN['id']."' AND group_name='$name' AND func_name='$fname'");
                }
                //-- mod_safe_mode end
  ...
  function save_config($item = "", $value) {
      ...
      global $ADMIN, $HTTP_POST_VARS, $INFO;
      $master[$item] = $value;
      $INFO = $ADMIN->rebuild_config($master);
  }

  function tools_build_tmpl_all() {
      global $IN, $root_path, $INFO, $DB, $SKIN, $ADMIN, $std, $MEMBER, $GROUP;
      if (defined("ROOT_PATH")) { $root_path = ROOT_PATH; }

      $this->save_config("resync_skins_start", $IN['resync_skins_start']);

      $dirnames = array();
      
      $fh = opendir($root_path."Skin");
      while (($dirname = readdir($fh)) !== false) {
          if (($dirname != ".") && ($dirname != "..") && is_dir($root_path."Skin/".$dirname) )
              $dirnames[++$i] = $dirname;
      }

      if (!count($dirnames)) 
          $ADMIN->done_screen("No Skins available", "Manage Template sets", "act=templ" );
      
      $this->save_config("resync_skins", count($dirnames));

      for ($i = 1; $i <= count($dirnames); $i++ ) {
          if ($i > $INFO['resync_skins_start']) {
              $IN['id'] = substr($dirnames[$i],1 );
              $this->tools_build_tmpl();
              $INFO['resync_skins_start']++;
              $this->save_config("resync_skins_start", $INFO['resync_skins_start']);
          }
          $this->html .= $SKIN->end_table();
      }
      $this->save_config("resync_skins", '0');
      $this->save_config("resync_skins_start", '0');
      $ADMIN->done_screen("All editable templates updated from source PHP skin files in all skin dirs", "Manage Template sets", "act=templ" );
      
  }

  function tools_rebuildphp_all() {
      global $IN, $root_path, $INFO, $DB, $SKIN, $ADMIN, $std, $MEMBER, $GROUP;
      if (defined("ROOT_PATH")) { $root_path = ROOT_PATH; }
  ...
  function tools_rebuildphp() {
      ...
      if (!is_dir($skin_dir)) {
          if (!@mkdir($skin_dir, 0755)) {
              if (SAFE_MODE_ON == 1) {
                  $ADMIN->error("$skin_dir doesn't exist. But we cannot create $skin_dir because of Safe Mode restriction".
                                         "<br>You have to create it yourself, CHMOD via FTP to 0755 or 0777 to enable this script to write into it.".
                                         " IBF cannot do this for you");
              } else {
                  $ADMIN->error("$skin_dir doesn't exist. But we cannot create $skin_dir because umknown reason.".
                                         "<br>You have to create it yourself, CHMOD via FTP to 0755 or 0777 to enable this script to write into it.".
                                         " IBF cannot do this for you");
              }
          }
      ...
      if (!is_writeable($skin_dir)) {
          $ADMIN->error("Cannot write in $skin_dir, CHMOD via FTP to 0755 or 0777 to enable this script to write into it. IBF cannot do this for you");
      }

      $groups = $DB->query("SELECT DISTINCT(group_name) FROM ibf_skin_templates WHERE set_id='".$IN['id']."'");

      if ( ! $DB->get_num_rows($groups) ) {
          $ADMIN->error("Rebuild Failed at template set creation: Can't query the information from the database");
      }

      while ( $trow = $DB->fetch_row($groups) ) {

          $output = "";
          $aq = $DB->query("SELECT * FROM ibf_skin_templates WHERE set_id='".$IN['id']."' AND group_name='".$trow['group_name']."'");
          ...
          $output .= "<?php\n\n";
          $output .= "class ".$trow['group_name']." {\n\n";

          while ( $next_row = $DB->fetch_row($aq) ) {
              $text = $next_row['section_content'];
              $text = str_replace( "\r\n", "\n", $text);
              $text = str_replace( "\n\n", "\n", $text);

              $output .= "function ".$next_row['func_name']."(".$next_row['func_data'].") {\n";
              $output .= "global \$ibforums;\n\n";
              $output .= "return <<<EOF\n";
              $output .= $text."\n";
              $output .= "EOF;\n";
              $output .= "}\n\n\n";
          ...
          $output .= "}\n\n?>";
          $FH = fopen($skin_dir."/".$trow['group_name'].".php", 'w');
          fwrite($FH, $output, strlen($output));
          fclose($FH);
          @chmod($skin_dir."/".$trow['group_name'].".php", 0777);
      }
      if ($IN['resync_skins'] != '1')
          $ADMIN->done_screen("Rebuild PHP Files successful from database templates", "Manage Template sets", "act=templ" );
  }
  ...
  //-- mod_safe_mode end
    ...
    $ADMIN->html .= $SKIN->add_td_row( array( $SKIN->form_dropdown( "tool",
                                                                                                 array(
                                                                                                         ...
                                                                                                         2 => array( 'php', 'Rebuild PHP skin files from database templates' ),
                                                                                                         )
                                                                                                         ) ,
                                                                        $SKIN->form_dropdown( "id", $form_array)
                                                                    ) );

    $ADMIN->html .= $SKIN->add_td_row( array( "<b>Manage all Skins ?</b>" ,
                                      $SKIN->form_yes_no( "resync_skins", '0' )
                             )      );

    $ADMIN->html .= $SKIN->add_td_row( array( "<b>Start with Skin No.?</b>" ,
                                     $SKIN->form_simple_input( 'resync_skins_start' , ($INFO['resync_skins_start']?$INFO['resync_skins_start']:'0'), 4 )
                             )      );
    if (FALSE)
    //-- mod_safe_mode end

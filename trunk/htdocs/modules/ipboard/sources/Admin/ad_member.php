...
class ad_forums {
	...
	function temp_ban_start()
	{
		...
		$DB->query("SELECT *, user_sig as signature FROM ibf_members WHERE uid='".$IN['mid']."'");
		...
		$ADMIN->html .= $SKIN->add_td_row( array( "<strong>Suspend {$member['uname']} for...</strong>" ,
	...
	function unsuspend()
	{
		...
		else
		{
			...
			$ADMIN->save_log("Unsuspended {$member['uname']}");

			$ADMIN->done_screen("{$member['uname']} Unsuspended", "Member Control", "act=mem" );
		}
	...
	function temp_ban_complete()
	{
		...
		if ($IN['send_email'] == 1)
		{
			...
			$msg = str_replace( "{membername}", $member['uname']       , $msg );
		...
		$ADMIN->html .= $SKIN->add_td_row( array( "<strong>{$member['uname']} is suspended until...</strong>",
		...
		$ADMIN->nav[] = array( "act=mem&code=stepone&USER_NAME={$member['uname']}", "Admin Options for {$member['uname']}" );

		$ADMIN->save_log("Suspended {$member['uname']} until ".$ADMIN->get_date( $show_ban['date_end'], 'SHORT'));
	...
	function domod() {
        ...
        // XOOPS Activation // Kou added
				$update = $DB->query("UPDATE ibf_members SET level=1, mgroup=".$row['real_group']." WHERE uid=".$row['uid']);
			...
			while ( $r = $DB->fetch_row() )
			{
				...
				$ADMIN->html .= $SKIN->add_td_row( array( "<b>".$r['uname']."</b>$coppa" ,
				 										  "<center>$where</center>",
												  		  ...
												  		  "<center><input type='checkbox' name='mid_{$r['uid']}' value='1'></center>"
											 )      );
			}
	...
	function delete_lookup_form() {
		...
		$DB->query("SELECT uid, uname FROM ibf_members WHERE uname LIKE '".$IN['USER_NAME']."%'");
	...
	function dodelete() {
		global $IN, $INFO, $DB, $SKIN, $ADMIN, $std, $MEMBER, $GROUP, $news_isactive;
		...
		//+--------------------------------------------
    //| By Koudanshi
		//| Convert their XOOPS's NEWS modules if available into guest postings...
		//+--------------------------------------------   
    if($news_isactive)
    {
		  $DB->query("UPDATE ".XOOPS_DB_PREFIX."_stories SET uid='0' WHERE uid='".$IN['MEMBER_ID']."'");
    }
    // --------------------------------------------
		// Delete member...
	...
	function do_add()
	{
		...
		//+--------------------------------------------
		//| Insert into XOOPS's right groups-Koudanshi
		//+--------------------------------------------

		if (($IN['mgroup'] == 4))
		{
		    $DB->query("INSERT INTO ".XOOPS_DB_PREFIX."_groups_users_link (groupid,uid) VALUES (1,'".$member_id."')");
		}

		$DB->query("INSERT INTO ".XOOPS_DB_PREFIX."_groups_users_link (groupid,uid) VALUES (2,'".$member_id."')");
	...
	function do_advanced_search($basic=0) {
		...
		if ($basic == 0)
		{
			...
			foreach( array('uname','email','ip_address','user_aim','user_icq','user_yim','user_sig','last_post','last_activity','mgroup') as $bit )
			{
		...
		while ( $r = $DB->fetch_row() )
		{
			...
			if ( $r['temp_ban'] != "" )
			{
				...
				$sus_link = " - <a href='{$SKIN->base_url}&act=mem&code=unsuspend&mid={$r['uid']}'>Unsuspend</a>";

				$tban = '&nbsp;&nbsp;<span style="font-size:10px">(Suspended until '.$ADMIN->get_date( $s_ban['date_end'], 'LONG') .$sus_link.')</span>';
			}

			$ADMIN->html .= $SKIN->add_td_basic( "<img src='html/sys-img/item.gif' border='0' alt='-'>&nbsp;<a style='font-size:12px' title='View this members profile' href='{$INFO['board_url']}/index.{$INFO['php_ext']}?act=Profile&MID={$r['uid']}' target='blank'>{$r['uname']}</a> $tban", "left", "pformstrip" );
			$ADMIN->html .= $SKIN->add_td_row( array( "{$r['ip_address']}",
													  $r['g_title'],
													  "<center>".$r['posts']."</center>",
													  "<center>".$r['email']."</center>",
													  "<center><strong><a href='{$SKIN->base_url}&act=mem&code=doform&MEMBER_ID={$r['uid']}' title='Edit this members account'>Edit Details</a></strong></center>",
													  "<center><a href='{$SKIN->base_url}&act=mem&code=changename&mid={$r['uid']}' title='Change this members name'>Change Name</a></center>",
													  "<center><a href='{$SKIN->base_url}&act=mem&code=banmember&mid={$r['uid']}' title='Suspend Member'>Suspend Account</a></span></center>"
									     	 )      );
	...
	function do_edit_form() {
		...
		$DB->query("SELECT *, user_sig as signature FROM ibf_members WHERE uid='".$IN['MEMBER_ID']."'");
		$mem = $DB->fetch_row();
		...
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Restrict {$mem['uname']} from posting?</b><br>Untick box and clear number box to remove restriction." ,
	...
	function do_edit()
	{
		...
		//+--------------------------------------------
		//| UPDATE XOOPS's right groups - Koudanshi
		//+--------------------------------------------

		if (($IN['mgroup'] != 4))
		{
		    $DB->query("DELETE FROM ".XOOPS_DB_PREFIX."_groups_users_link WHERE uid='".$IN['mid']."' AND groupid='1'");
		}
		else
		{
		    $DB->query("INSERT INTO ".XOOPS_DB_PREFIX."_groups_users_link (groupid,uid) VALUES (1,'".$IN['mid']."')");
		}
		//--------------------------------------------
		...
		if ( $IN['remove_photo'] )
		{
			$DB->query("SELECT uid FROM ibf_member_extra WHERE uid={$IN['mid']}");

...
class Forums {
	...
	function process_forum($forum_id="", $forum_data="")
  {
  	global $std, $ibforums, $INFO;
		...
		{
      $forum_data['last_poster_name'] = strlen($forum_data['last_poster_name']) > $INFO['xbbc_uname_len']? substr($forum_data['last_poster_name'],0,$INFO['xbbc_uname_len']).".." : $forum_data['last_poster_name'];//Kou added
    ...
    $r_sort_by = $sort_by == 'A-Z' ? 'ASC' : 'DESC';
		...
		$this->forum['SHOW_PAGES'] = $std->build_pagelinks( array( 'TOTAL_POSS'  => $total_possible['max'],
		...
		if ( ($ibforums->vars['show_user_posted'] == 1) and ($ibforums->member['uid']) )
		{
			$query = "SELECT DISTINCT(ibf_posts.author_id), ibf_topics.* FROM ibf_topics
			           LEFT JOIN ibf_posts ON
			           (ibf_topics.tid=ibf_posts.topic_id AND ibf_posts.author_id=".$ibforums->member['uid'].")
	...
	function render_entry($topic) {
		global $DB, $std, $ibforums, $INFO;

		$topic['last_text']   = $ibforums->lang['last_post_by'];
    $topic['last_poster_name'] = strlen($topic['last_poster_name']) > $INFO['xbbc_uname_len']? substr($topic['last_poster_name'],0,$INFO['xbbc_uname_len']).".." : $topic['last_poster_name'];//Kou added
    ...
    $topic['starter_name'] = strlen($topic['starter_name']) > $INFO['xbbc_uname_len']? substr($topic['starter_name'],0,$INFO['xbbc_uname_len']).".." : $topic['starter_name'];//Kou added

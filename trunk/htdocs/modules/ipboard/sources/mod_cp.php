...
class Moderate {
	...
	function do_ip()
	{
		...
		else if ($ibforums->input['iptool'] == 'members')
		{
			...
			$DB->query("SELECT count(uid) as max FROM ibf_members WHERE $sql");
			...
			$DB->query("SELECT uname as name, uid as id, ip_address, posts, user_regdate as joined FROM ibf_members WHERE $sql ORDER BY user_regdate DESC LIMIT {$this->start_val},50");
	...
	function approve_all()
	{
		...
		if ( count($mems) > 0 )
		{
			if ( $this->forum['inc_postcount'] )
			{
				...
				$loopy_loo = $DB->query("SELECT uid, mgroup, posts FROM ibf_members WHERE uid IN ($mstring)");
						...
						if ( $gid > 0 and $gposts > 0 )
						{
							if ( $member['posts'] + 1 >= $gposts )
							{
								...
								if ( USE_MODULES == 1 )
								{
									...
									$this->modules->on_group_change($ibforums->member['uid'], $gid);
								}
							}
						}
					}

					$newbie = $DB->query("UPDATE ibf_members SET ".$mgroup."posts=posts+1 WHERE uid={$member['uid']}");
				}
			}
		}
	...
	function mod_topicview()
	{
		...
		$DB->query( "SELECT p.*, ".
				    "m.uid,m.uname,m.mgroup,m.email,m.joined,m.user_avatar,m.avatar_size,m.posts as member_posts,m.aim_name,m.icq_number,m.user_sig as signature, m.url,m.user_yim,m.title,m.hide_email,m.msnname, ".
				    "g.g_id, g.g_title, g.g_icon, t.* ".
				    "FROM ibf_posts p, ibf_members m, ibf_groups g, ibf_topics t ".
				    "WHERE t.tid='".$ibforums->input['tid']."' AND t.approved=1 AND p.topic_id=t.tid AND p.queued=1 AND p.author_id=m.uid AND g.g_id=m.mgroup ".
				    "ORDER BY p.pid ASC");
	...
	function mod_topics()
	{
		...
		$DB->query( "SELECT p.*, ".
				    ...
				    "g.g_id, g.g_title, g.g_icon, t.* ".
				    "FROM ibf_posts p, ibf_members m, ibf_groups g, ibf_topics t ".
				    "WHERE t.forum_id='".$this->forum['id']."' and t.approved=0 AND p.topic_id=t.tid AND p.new_topic=1 AND p.author_id=m.uid AND g.g_id=m.mgroup ".
	...
	function prune_juice()
	{
		...
		if ($ibforums->input['check'] == 1)
		{
			...
			{
				$DB->query("SELECT uid FROM ibf_members WHERE uname='".$ibforums->input['member']."'");
	...
	function find_user_two()
	{
		...
		$DB->query("SELECT uid, uname FROM ibf_members WHERE uname LIKE '".$ibforums->input['name']."%' LIMIT 0,100");
	...
	function edit_user()
	{
		...
		$DB->query("SELECT m.*, g.* FROM ibf_members m, ibf_groups g WHERE m.uid='".$ibforums->input['memberid']."' AND m.mgroup=g.g_id");
		...
		$editable['signature'] = $parser->unconvert($member['user_sig']);
	...
	function complete_user_edit()
	{
		...
		$profile = array (
						   'user_sig'    => $ibforums->input['signature'],
		...
		if ($ibforums->input['avatar'] == 1)
		{
			$profile['user_avatar']      = "";
			$profile['avatar_size'] = "";
			$this->bash_uploaded_avatars($member['uid'], $ibforums->member['uname']);
		}

		if ($ibforums->input['photo'] == 1)
		{
			$this->bash_uploaded_photos($member['uid']);

			$DB->query("SELECT id FROM ibf_member_extra WHERE id={$member['uid']}");

			if ( $DB->get_num_rows() )
			{
				$DB->query("UPDATE ibf_member_extra SET photo_location='', photo_type='', photo_dimensions='' WHERE id={$member['uid']}");
			}
			else
			{
				$DB->query("INSERT INTO ibf_member_extra SET photo_location='', photo_type='', photo_dimensions='', id={$member['uid']}");
			}
		...
		$DB->query("UPDATE ibf_members SET $db_string WHERE uid='".$ibforums->input['memberid']."'");
	...
	function bash_uploaded_avatars($id, $name)
	{
		...
		foreach( array( 'swf', 'jpg', 'jpeg', 'gif', 'png' ) as $ext )
		{
			if ( @file_exists( $ibforums->vars['upload_dir']."/cavt-".$id."-".$name.".".$ext ) )
			{
				@unlink( $ibforums->vars['upload_dir']."/cavt-".$id."-".$name.".".$ext );
			}
		}
	}

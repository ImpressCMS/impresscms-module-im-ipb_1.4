...
class Boards {
    ...
    function Boards()
    {
		...
		if ($ibforums->vars['show_active'])
		{
			...
			{
				if ( strstr( $result['sess_id'], '_session' ) )
				{
			...
			$year  = $a[0];
				...
				$DB->query("SELECT uid, uname, bday_day as DAY, bday_month as MONTH, bday_year as YEAR
						    FROM ibf_members WHERE bday_day=$day and bday_month=$month");

				while ($user = $DB->fetch_row())
				{
					$birthstring .= "<a href='{$this->base_url}showuser={$user['uid']}'>{$user['uname']}</a>";
		...
		if ($ibforums->vars['show_calendar'])
		{
			...
			{
				if ($event['priv_event'] == 1 and $ibforums->member['uid'] != $event['userid'])
				{
					continue;
				}
    ...
    function process_forum($forum_id="", $forum_data="")
    {
    	...
    	if ($forum_data['subwrap'] == 1)
    	{
    		...
    		if ( (isset($this->children[ $forum_data['id'] ])) and (count($this->children[ $forum_data['id'] ]) > 0 ) )
    		{
				...
				{
          $newest['last_poster_name'] = strlen($newest['last_poster_name']) > $INFO['xbbc_uname_len']? substr($newest['last_poster_name'],0,$INFO['xbbc_uname_len']).".." : $newest['last_poster_name'];//Kou added           
    	...
    	else
    	{
			...
			if (isset($forum_data['last_title']) and $forum_data['last_id'])
			{
				...
				if (strlen($forum_data['last_title']) > 20)
				{
					$forum_data['last_title'] = substr($forum_data['last_title'],0,17) . "...";

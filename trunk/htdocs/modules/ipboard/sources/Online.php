...
class Online {
	...
	function list_all() {
		...
		$db_key   = $sort_key_value   == 'click' ? 'sess_updated' : 'member_name';

		switch ($show_mem_value)
		{
			case 'reg':
				$db_mem = " AND (s.member_group <> {$ibforums->vars['guest_group']} OR s.sess_id LIKE '%session' ) ";
				break;
			case 'guest':
		...
		$DB->query("SELECT COUNT(sess_id) as total_sessions FROM ibf_sessions s WHERE login_type <> 1 AND sess_updated > $t_time".$db_mem);
		$max = $DB->fetch_row();
		...
		$DB->query("SELECT s.sess_id, s.in_forum, s.in_topic, s.member_name, s.member_id, s.sess_ip, s.sess_updated, s.location,
						   s.login_type, s.member_group, g.prefix, g.suffix
				    FROM ibf_sessions s, ibf_groups g
				    WHERE s.sess_updated > $t_time AND s.member_group=g.g_id $db_mem ORDER BY $db_key $db_order LIMIT ".$this->first.",25");
		...
		foreach( $final as $idx => $sess )
		{
			...
			if ( strstr( $sess['sess_id'], '_session' ) )
			{
				$sess['is_bot'] = 1;

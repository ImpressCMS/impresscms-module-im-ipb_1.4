...
class Memberlist {
    ...
    function Memberlist()
    {
    	...
    	$the_sort_key = array( 'uname'          => 'sort_by_name',
    						   'posts'         => 'sort_by_posts',
    	...
    	{
    		$DB->query("SELECT COUNT(m.uid) as total_members FROM ibf_members m
                        LEFT JOIN ibf_member_extra me ON me.id=m.uid
    				    LEFT JOIN ibf_groups g ON m.mgroup=g.g_id
    				    WHERE me.photo_location <> '' AND m.uid > 0  AND g.g_hide_from_list <> 1".$q_extra);
    	...
    	else
    	{
	    	$DB->query("SELECT COUNT(m.uid) as total_members FROM ibf_members m
	    				LEFT JOIN ibf_groups g ON m.mgroup=g.g_id
	    				WHERE m.uid > 0  AND g.g_hide_from_list <> 1".$q_extra);
	    }
		...
		$DB->query("SELECT m.uname, m.uid, m.posts, m.user_regdate, m.mgroup, m.email,m.title, m.user_viewemail, m.user_from, m.user_aim, m.user_icq,
		...
		while($member = $DB->fetch_row() )
		{
			...
			if ($member['user_icq'])
			{
				$member['icq_number'] = "<a href=\"javascript:PopUp('{$this->base_url}act=ICQ&amp;MID={$member['uid']}','Pager','450','330','0','1','1','1')\"><{P_ICQ}></a>";
			}
			else
			{
				$member['icq_number'] = '&nbsp;';
			...
			if ($member['user_aim'])
			{
				$member['aim_name'] = "<a href=\"javascript:PopUp('{$this->base_url}act=AOL&amp;MID={$member['uid']}','Pager','450','330','0','1','1','1')\"><{P_AOL}></a>";
			}
			else
			{
				$member['aim_name'] = '&nbsp;';

...
class post_functions extends Post {
	...
	function post_functions($class) {
		...
		{
			$DB->query("SELECT member_id, member_name, mid, edit_post, edit_topic FROM ibf_moderators WHERE forum_id=".$class->forum['id']." AND (member_id=".$ibforums->member['uid']." OR (is_group=1 AND group_id='".$ibforums->member['mgroup']."'))");
		...
		if ( ($this->orig_post['author_id'] == $ibforums->member['uid']) and ($ibforums->member['g_edit_posts']) )
		{
	...
	function complete_edit($class) {
		...
		$this->post['edit_name']   = $ibforums->member['uname'];

...
class modfunctions
{
	...
	function topic_add_reply($post="", $tids=array(), $incpost=0)
	{
		...
		$post = array(
					  ...
					  'icon_id'     => 0,
					  'post'        => $post,
					  'author_name' => $ibforums->member['uname'],
					  'forum_id'    => "",
					  'topic_id'    => "",
					  'queued'      => 0,
					  'attach_id'   => "",
		...
		foreach( $tids as $row )
		{
			...
			if ( $tid and $fid )
			{
				...
				$ta = array (
							  ...
							  'last_poster_name' => $ibforums->member['uname'],
		...
		if ( $add_posts > 0 )
		{
			...
			$DB->query("UPDATE ibf_members SET ".$pcount.$mgroup."last_post=".time()." WHERE uid=".$ibforums->member['uid']);
	...
	function topic_move($topics, $source, $moveto, $leavelink=0)
	{
		...
		$DB->query("SELECT tr.*, m.uid, m.mgroup, m.org_perm_id, f.read_perms, f.id, t.tid, g.g_id, g.g_perm_id
				     FROM ibf_tracker tr
				     LEFT JOIN ibf_topics t ON (tr.topic_id=t.tid)
				     LEFT JOIN ibf_forums f ON (t.forum_id=f.id)
				     LEFT JOIN ibf_members m on (m.uid=tr.member_id)

...
EOF;
    ...
    function do_log_in() {
		...
		$DB->query("SELECT uid, uname, mgroup, pass, new_pass FROM ibf_members WHERE LOWER(uname)='$username'");
		...
		if ($DB->get_num_rows())
		{
			...
			if ($ibforums->input['s'])
			{
				...
				$DB->query("DELETE FROM ibf_sessions WHERE ip_address='".$ibforums->input['IP_ADDRESS']."' AND sess_id <> '$session_id'");
			...
			$DB->query("UPDATE ibf_members SET pass='$real_pass' WHERE uid='".$member['uid']."'");
	...
	function do_log_out() {
		...
		$DB->query("UPDATE ibf_sessions SET ".
				     ...
				     "member_pass='NULL',".
				     "login_type='0' ".
				     "WHERE sess_id='". $ibforums->session_id ."'");

...
class Contact {
	...
	function show_integ()
	{
		...
		$DB->query("SELECT uname, uid, integ_msg from ibf_members WHERE uid='".$ibforums->input['MID']."'");
	...
	function chat_display()
	{
		...
		{
			$user = $ibforums->member['uname'];
			$pass = $ibforums->member['pass'];
		}
	...
	function send_report()
	{
		...
		$DB->query("SELECT m.uname, m.email, mod.member_id FROM ibf_moderators `ipbmods`, ibf_members `m` WHERE ipbmods.forum_id='$fid' and ipbmods.member_id=m.uid");
		...
		else
		{
			...
			$DB->query("SELECT m.uid, m.uname, m.email FROM ibf_members `m`, ibf_groups `g` WHERE g.g_access_cp=1 AND m.mgroup=g.g_id");
	...
	function show_msn() {
		...
		$DB->query("SELECT uname, uid, user_msnm FROM ibf_members WHERE uid='".$ibforums->input['MID']."'");
	...
	function show_yahoo() {
		...
		$DB->query("SELECT uname, uid, user_yim FROM ibf_members WHERE uid='".$ibforums->input['MID']."'");
	...
	function show_aim() {
		...
		$DB->query("SELECT uname, uid, user_aim FROM ibf_members WHERE uid='".$ibforums->input['MID']."'");
		...
		$print->pop_up_window( "AOL CONSOLE", $this->html->aol_body( array( 'AOLNAME' => $member['user_aim'] ) ) );
	...
	function show_icq() {
		...
		$DB->query("SELECT uname, uid, user_icq FROM ibf_members WHERE uid='".$ibforums->input['MID']."'");
		...
		$html .= $this->html->icq_body( array( 'UIN' => $member['user_icq'] ) );
	...
	function mail_member_form($errors="", $extra = "")
	{
		...
		$DB->query("SELECT uname, uid, email, user_viewemail from ibf_members WHERE uid=".$ibforums->input['MID']);
	...
	function mail_member_send()
	{
		...
		$DB->query("SELECT uname, uid, email, user_viewemail FROM ibf_members WHERE uid=".$ibforums->input['to']);
		...
		$email_check = $this->_allow_to_email( $ibforums->member['uid'], $ibforums->member['g_email_limit'] );

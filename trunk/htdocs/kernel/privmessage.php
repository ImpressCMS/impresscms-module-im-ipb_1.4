<?php
// $Id: privmessage.php 507 2006-05-26 23:39:35Z skalpa $
/*
// ------------------------------------------------------------------------
-+ Date: 12-SEP-2006
 ...
 *
 * @version		$Revision: 507 $ - $Date: 2006-05-27 01:39:35 +0200 (Sat, 27 May 2006) $
 */
 ...
 **/
    function insert(&$pm)
    {
        ...
        //------------------------------------------------
        // IPBM's PM compatibility
        // Koudanshi
        //------------------------------------------------
        if ($isbb) {        
          if ($pm->isNew()) {
              $sql = sprintf("INSERT INTO %s (title, from_id, member_id, recipient_id, msg_date, message, vid, read_state) VALUES (%s, %u, %u, %u, %u, %s, %s, %u)", $this->db->prefix('ipb_messages'), $this->db->quoteString($subject), $from_userid, $to_userid, $to_userid, time(), $this->db->quoteString($msg_text),  $this->db->quoteString("in"),0);
              if (!$result = $this->db->query($sql)) {
                  return false;
              }
              if (empty($msg_id)) {
              ...
              $this->db->queryF("UPDATE ".$this->db->prefix('users')." SET msg_total = msg_total + 1, new_msg = new_msg + 1, msg_from_id='".$from_userid."', msg_msg_id='".$msg_id."', show_popup='".$showpopup_bb."' WHERE uid='".$to_userid."' ");
          } //else {

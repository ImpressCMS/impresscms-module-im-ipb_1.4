<?php
// $Id: member.php 558 2006-06-20 06:35:23Z skalpa $
/*
// ------------------------------------------------------------------------
-+ Date: 12-SEP-2006
...
class XoopsMemberHandler{
    ...
    */
    ...
    function addUserToGroup($group_id, $user_id)
    {
        ...
        //<<<-----------------------------------------
        //-+- IPBM Members right update	-- Koudanshi
        //<<<-----------------------------------------

        if ($isbb)
        {
            if ($group_id == 1) $mgroup = 4;
            else if ($group_id == 3) $mgroup = 2;
            else $mgroup = 3;
            $xoopsDB->query("UPDATE ".$xoopsDB->prefix('users')." SET mgroup = $mgroup WHERE uid = $user_id");
        }
        //>>>-----------------------------------------
    ...
    function removeUsersFromGroup($group_id, $user_ids = array())
    {
        ...
        foreach ($user_ids as $uid) {
	        ...
	        if ($isbb)
	        {
	            $xoopsDB->query("UPDATE ".$xoopsDB->prefix('users')." SET mgroup = '3' WHERE uid = $uid");
	        }
	        //>>>-----------------------------------------

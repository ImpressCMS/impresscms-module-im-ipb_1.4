<?php
// $Id: auth_ldap.php 512 2006-05-27 01:16:37Z skalpa $
...
// ------ added by m0nty for X-IPB 1.4J
    /**
	 *  AuthenticateMd5  user again LDAP directory (Bind)
	 *  2 options : 
	 * 		Authenticate directly with uname in the DN
	 * 		Authenticate with manager, search the dn
	 *
	 * @param string $uname Username
	 * @param string $md5pwd Password
	 *
	 * @return bool
	 */	
    function authenticatemd5($uname, $md5pwd = null) {
        $authenticated = false;
        if (!extension_loaded('ldap')) {
            $this->setErrors(0, _AUTH_LDAP_EXTENSION_NOT_LOAD);
            return $authenticated;
        ...
        if ($this->_ds) {
            ...
            if (!$userDN) return false;
            // We bind as user to test the credentials         
            $authenticated = ldap_bind($this->_ds, $userDN, stripslashes($md5pwd));
            if ($authenticated) {
            	// We load the Xoops User database
            	return $this->loadXoopsUser($userDN, $uname, $md5pwd);            	               
            } else $this->setErrors(ldap_errno($this->_ds), ldap_err2str(ldap_errno($this->_ds)) . '(' . $userDN . ')');
        }
        else {
            $this->setErrors(0, _AUTH_LDAP_SERVER_NOT_FOUND);            
...
// ------ end X-IPB 1.4J
	...
	function loadXoopsUser($userdn, $uname, $pwd = null) {
		$xoopsUser = XoopsAuthProvisionning::getXoopsUser($uname);		
		if (!$xoopsUser) {
			if ($this->ldap_provisionning) { 
				$newuser = $this->createXoopsUser($userdn, $uname, $pwd);
				return $newuser;
			} else $this->setErrors(0, sprintf(_AUTH_LDAP_XOOPS_USER_NOTFOUND, $uname));
		}
		return $xoopsUser;
	}
    
    /**
	 *  Create user in the XOOPS Database
	 * 
	 *
	 * @return userDN or false
	 */	        
    function createXoopsUser($dn, $uname, $pwd = null) {
    	$newuser = false;
		include_once ICMS_ROOT_PATH . '/class/auth/auth_provisionning.php';
		$provisHandler = XoopsAuthProvisionning::getInstance();    	
    	// $member_handler =& xoops_gethandler('member');
        /* if (!ldap_bind($this->_ds, $this->ldap_manager_dn, stripslashes($this->ldap_manager_pass))) {
            $this->setErrors(ldap_errno($this->_ds), ldap_err2str(ldap_errno($this->_ds)) . '(' . $this->ldap_manager_dn . ')');
            return $ret;
        }*/
        $sr = ldap_read($this->_ds, $dn, '(objectclass=*)');
        $entries = ldap_get_entries($this->_ds, $sr);        
        if ($entries["count"] > 0) {
        	$newuser = $provisHandler->sync($entries[0], $uname, $pwd);
        }
        else $this->setErrors(0, sprintf(_AUTH_LDAP_CANT_READ_ENTRY, $dn));
        return $newuser;
    }

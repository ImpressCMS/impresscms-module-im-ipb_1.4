<?php
// $Id: auth_ads.php 512 2006-05-27 01:16:37Z skalpa $
...
class XoopsAuthAds extends XoopsAuthLdap {
    ...
    function authenticate($uname, $pwd = null) {
        ...
        if ($this->_ds) {
            ...
            $authenticated = ldap_bind($this->_ds, $userUPN, stripslashes($pwd));
            if ($authenticated) {
...
// ------ added by m0nty for X-IPB 1.4J
    ...
    function authenticatemd5($uname, $md5pwd = null) {
        ...
        if ($this->_ds) {
            ...
            if (!$userUPN) return false;
            // We bind as user to test the credentials         
            $authenticated = ldap_bind($this->_ds, $userUPN, stripslashes($md5pwd));
            if ($authenticated) {
            	// We load the Xoops User database
            	$dn = $this->getUserDN($uname);
            	return $this->loadXoopsUser($dn, $uname, $md5pwd);                
            } else $this->setErrors(ldap_errno($this->_ds), ldap_err2str(ldap_errno($this->_ds)) . '(' . $userUPN . ')');
        }
        else {
            $this->setErrors(0, _AUTH_LDAP_SERVER_NOT_FOUND);              

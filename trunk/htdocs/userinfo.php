...
if(!$xoopsConfigUser['allow_annon_view_prof'] && !is_object($xoopsUser)) {redirect_header('index.php', 3, _NOPERM);}

$uid = intval($_GET['uid']);
if($uid <= 0) {redirect_header('index.php', 3, _US_SELECTNG);}
...
if(is_object($xoopsUser))
{
	if ($uid == intval($xoopsUser->getVar('uid')))
	{
		...
		if($isbb && !$INFO['xbbc_ucp'])
		{
			@header('Location: '.ICMS_URL.'/modules/ipboard/index.php?showuser='.$uid);
			exit();
		}
		...
		else {$xoopsTpl->assign('user_candelete', false);}
		$thisUser = & $xoopsUser;
	}
	else
	{
		...
		if(!is_object($thisUser) || !$thisUser->isActive()) {redirect_header('index.php', 3, _US_SELECTNG);}
		// Im-IPB Core edit - use ICMS CP or Im-IPB
		if($isbb && !$INFO['xbbc_ucp'])
		{
			@header('Location: '.ICMS_URL.'/modules/ipboard/index.php?showuser='.$uid);
...
$xoopsTpl->assign('lang_avatar', _US_AVATAR);
if($xoopsConfigUser['avatar_allow_gravatar'] == 1) {$xoopsTpl->assign('user_avatarurl', $thisUser->gravatar('G', $xoopsConfigUser['avatar_width']));}
else {$xoopsTpl->assign('user_avatarurl', ICMS_UPLOAD_URL.'/'.$thisUser->getVar('user_avatar'));}
$xoopsTpl->assign('lang_realname', _US_REALNAME);
$xoopsTpl->assign('user_realname', $thisUser->getVar('name'));
$xoopsTpl->assign('lang_website', _US_WEBSITE);
if($thisUser->getVar('url', 'E') == '') {$xoopsTpl->assign('user_websiteurl', '');}
else {$xoopsTpl->assign('user_websiteurl', '<a href="'.$thisUser->getVar('url', 'E').'" rel="external">'.$thisUser->getVar('url').'</a>');}
...
else {$xoopsTpl->assign('user_alwopenid', false);}
...
if($thisUser->getVar('user_viewemail') == 1) {$xoopsTpl->assign('user_email', $thisUser->getVar('email', 'E'));}
else
{
	if(is_object($xoopsUser))
	{
		...
		else {$xoopsTpl->assign('user_email', '&nbsp;');}
	}
}
if($icmsauthConfig['auth_openid'] == 1)
{
	if($thisUser->getVar('user_viewoid') == 1) {$xoopsTpl->assign('user_openid', $thisUser->getVar('openid', 'E'));}
	else
	{
		if(is_object($xoopsUser))
		{
			...
			else {$xoopsTpl->assign('user_openid', '&nbsp;');}
		}
	}
}
if(is_object($xoopsUser))
...
else {$xoopsTpl->assign('user_pmlink', '');}
$userrank = $thisUser->rank();
if($userrank['image']) {$xoopsTpl->assign('user_rankimage', '<img src="'.ICMS_UPLOAD_URL.'/'.$userrank['image'].'" alt="" />');}
$xoopsTpl->assign('user_ranktitle', $userrank['title']);
$date = $thisUser->getVar('last_login');
if(!empty($date)) {$xoopsTpl->assign('user_lastlogin', formatTimestamp($date, 'm'));}
	...
	if($gperm_handler->checkRight('module_read', $mid, $groups))
	{
		...
		if(is_array($results) && $count > 0)
		{
			for($i = 0; $i < $count; $i++)
			{
				...
				else {$results[$i]['image'] = 'images/icons/posticon2.gif';}
